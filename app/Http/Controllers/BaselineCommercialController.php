...
class BaselineCommercialController extends Controller
{
     ...
     * from the pinned rate card version. The replace runs under the draft
     * lock so concurrent saves cannot merge and a submission cannot slip
     * in between delete and insert.
     */
    public function update(UpdateBaselineCommercialsRequest $request, Baseline $baseline): RedirectResponse
    {
        ...
        $baseline->mutateAsDraft(function () use ($baseline, $allocations): void {
